...
get_inbound_suffix() {
    ...
    local nft_rule
    nft_rule=$(nft list ruleset 2>/dev/null | grep -E 'tcp dport \{?\s*(7891|7892|7893|7894|7895|7896|7901|7601|7701|7700|7702|7703|7704|7705)\s*\}? accept' || true)
    if [[ $nft_rule =~ "tcp dport 7891 accept" || $nft_rule =~ "tcp dport { 7891" ]]; then
        ...
        echo "inbound_ru"
    elif [[ $nft_rule =~ "tcp dport 7700 accept" || $nft_rule =~ "tcp dport { 7700" ]]; then
        echo "inbound_1_xhttp"
    elif [[ $nft_rule =~ "tcp dport 7702 accept" || $nft_rule =~ "tcp dport { 7702" ]]; then
